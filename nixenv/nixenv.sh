...
function eflbuild() {
    if [ $WEBKITSOURCEDIR ]; then
        ...
        cd $WEBKITSOURCEDIR
        rm -rf $WEBKITOUTPUTDIR/$WKBUILD
        Tools/Scripts/build-webkit --efl --${WKBUILD,,} --makeargs="-j80" --prefix=$WEBKITINSTALLDIR $FLAGS
    else
        echo "[ERROR] Please set the efl environment."
    fi
}
...
function gtkbuild() {
    if [ $WEBKITSOURCEDIR ]; then
        ...
        cd $WEBKITSOURCEDIR
        rm -rf $WEBKITOUTPUTDIR/$WKBUILD
        Tools/Scripts/build-webkit --gtk --${WKBUILD,,} --makeargs="-j80" --prefix=$WEBKITINSTALLDIR $FLAGS
    else
        echo "[ERROR] Please set the gtk environment."
    fi
}
